...
async function checkPetStatus(interaction, petName, now) {
  try {
    ...
    const lastVetVisitTime = petDb.actionTimeStamp.lastVetVisit?.length
      ? new Date(
          petDb.actionTimeStamp.lastVetVisit[
            petDb.actionTimeStamp.lastVetVisit.length - 1
          ]
        ).getTime()
      : 0;
    ...
    if (petDb.isAsleep && (sleepUntilTime && now < sleepUntilTime)) {
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.followUp(
          `${StringpetName} was woken up from their slumber and is a bit groggy. Energy, happiness, and sleep level have decreased slightly.`
        );
      } else {
        await interaction.reply(
    ...
    if (hasSleepUntilPassed) {
      ...
      petDb.happiness = Math.max(
        petDb.happiness - petDb.happiness * 0.05,
        0
      );
      petDb.sleepLevel = Math.max(
        petDb.sleepLevel - petDb.sleepLevel * 0.05,
        0
      );
