...
module.exports = {
  structure: new SlashCommandBuilder().setName('devsupport').setDescription('Creates a form to send a support request to PawPal developers'),
  /**
  ...
  run: async (client, interaction) => {
    try {
      ...
      const guildIdInput = new TextInputBuilder().setLabel('Guild ID').setCustomId('guild_id_input').setPlaceholder('Guild ID').setStyle(TextInputStyle.Short).setRequired(true).setValue(guildId);
      ...
      const emailInput = new TextInputBuilder().setLabel('Contact Email').setCustomId('email_input').setPlaceholder('Enter your email...').setStyle(TextInputStyle.Short).setRequired(true);
      ...
      const modal = new ModalBuilder().setTitle('Developer Support').setCustomId('dev-support-modal').addComponents(guildNameActionRow, guildIdActionRow, issueActionRow, emailActionRow);
