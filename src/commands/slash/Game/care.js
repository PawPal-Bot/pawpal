const {
  ...
  async function handleClean(interaction, petDb, petName, now, randomPetSound) {
    ...
    if (!petDb.actionTimeStamp.lastCleaned)
      petDb.actionTimeStamp.lastCleaned = [];
    ...
    const lastCleanedTime =
      petDb.actionTimeStamp.lastCleaned.length > 0
        ? new Date(petDb.actionTimeStamp.lastCleaned.slice(-1)[0]).getTime()
        : 0;
    const lastGroomedTime =
      petDb.actionTimeStamp.lastGroomed.length > 0
        ? new Date(petDb.actionTimeStamp.lastGroomed.slice(-1)[0]).getTime()
        : 0;
  ...
  async function handleGroom(interaction, petDb, petName, now, randomPetSound) {
    if (!petDb.actionTimeStamp) petDb.actionTimeStamp = {};
    if (!petDb.actionTimeStamp.lastGroomed)
      petDb.actionTimeStamp.lastGroomed = [];
    if (!petDb.actionTimeStamp.lastCleaned)
      petDb.actionTimeStamp.lastCleaned = [];
    ...
    petDb.actionTimeStamp.lastGroomed.push(now.toISOString());
  ...
  async function handleFeed(interaction, petDb, petName) {
    ...
    const canFeedFood = checkIfCanFeed(
      petDb.actionTimeStamp.lastFed,
      petDb.hunger
    );
  
  
    const canFeedWater = checkIfCanFeed(
      petDb.actionTimeStamp.lastDrank,
      petDb.thirst
    );
  ...
  async function handleVet(interaction, petDb, petName, now) {
    if (!petDb.actionTimeStamp) petDb.actionTimeStamp = {};
    if (!petDb.actionTimeStamp.lastVetVisit)
      petDb.actionTimeStamp.lastVetVisit = [];
