...
const checkPetStatus = require("../../../utils/eventChecks");
  ...
  module.exports = {
    structure: new SlashCommandBuilder()
      ...
      .addSubcommand((subcommand) =>
        subcommand
          .setName("hideandseek")
          .setDescription("Your pet is hiding, can you find them?")
      ),
      /**
  ...
  async function handlePat(interaction, petDb, petName, now, randomPetSound) {
    ...
    const recentPats = petDb.actionTimeStamp.lastPat.filter(
...
async function handleHideAndSeek(interaction, petDb, petName, now, randomPetSound) {
    ...
    await petProfile.findOneAndUpdate({ userId: interaction.user.id }, {
        $set: {
    ...
    const components = buttonComponents.map(comp => 
        new ButtonBuilder()
