...
module.exports = {
  structure: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand => subcommand.setName("started").setDescription("Commence the initial setup for PawPal.")),
  options: {
    cooldown: 5000,
  },
  /**
  ...
  run: async (client, interaction, petDb) => {
    ...
    try {
      petDb = await petProfile.findOneAndUpdate({ userId }, {}, { upsert: true, new: true, setDefaultsOnInsert: true }).exec();
    } catch (error) {
