...
function createBasicInfoEmbed(petProfile, imageUrl) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '**Pet Details**', value: `\`🐾 Type:\` ${petTypeToString(petProfile.petType)}\n\`🌟 Life Stage:\` ${lifeStageToString(petProfile.lifeStage)}\n\`🎂 Age:\` ${petProfile.age} days old`, inline: false },
      { name: '**Current State**', value: `\`💕 Affection:\` ${variables.getAffection(petProfile.affection)} (${scaledAffection})\n\`😄 Happiness:\` ${variables.getHappiness(petProfile.happiness)} (${scaledHappiness})\n\`😴 Energy:\` ${variables.getEnergy(petProfile.energy)} (${scaledEnergy})`, inline: false },
      { name: '**Health Status**', value: `\`🤒 Is Sick:\` ${petProfile.isSick ? 'Yes' : 'No'}`, inline: false },
      //{ name: '**Training Level**', value: `\`🏋️‍♂️ Training:\` ${petProfile.trainingLevel}`, inline: false },
      { name: '**Exercise Level**', value: `\`🏃 Exercise:\` ${petProfile.exerciseLevel}`, inline: false },
      { name: '**Education Level**', value: `\`🎓 Education:\` ${petProfile.educationLevel}`, inline: false }
    )
    .setColor('#9e38fe');
}
...
function createHealthNeedsEmbed(petProfile, imageUrl) {
  ...
  return new EmbedBuilder()
    ...
    .setThumbnail(imageUrl)
    .addFields(
      { name: '**Health & Needs**', value: `\`❤️ Health:\` ${variables.getHealth(petProfile.health)} (${scaledHealth})\n\`🍽️ Hunger:\` ${variables.getHunger(petProfile.hunger)} (${scaledHunger})\n\`💧 Thirst:\` ${variables.getThirst(petProfile.thirst)} (${scaledThirst})\n\`🚿 Cleanliness:\` ${variables.getCleanliness(petProfile.cleanliness)} (${scaledCleanliness})\n\`🔋 Energy:\` ${variables.getEnergy(petProfile.energy)} (${scaledEnergy})`, inline: false },
      { name: '**Additional Info**', value: `\`💊 Medicine Count:\` ${petProfile.medicineCount}\n\`💪 Discipline Level:\` ${petProfile.discipline}`, inline: false },
      { name: '**Rest & Recovery**', value: `\`💤 Sleep Level:\` ${petProfile.sleepLevel}\n\`💊 Last Medicine:\` ${getLastTimeStamp(petProfile.actionTimeStamp.lastMedicine)}`, inline: false },
      { name: '**Hunting & Play**', value: `\`🏹 Hunt Count:\` ${petProfile.huntCount}\n\`🎮 Mini Game Scores:\` coming soon`, inline: false }
    )
    .setColor('#9e38fe');
}
...
function createInteractionsEmbed(petProfile, imageUrl) {
  return new EmbedBuilder()
    ...
    .setThumbnail(imageUrl)
    .addFields(
      { name: '**Interactions**', value: `\`👋 Pat Count:\` ${petProfile.patCount}\n\`🤱 Cuddle Count:\` ${petProfile.cuddleCount}\n\`🍽️ Feed Count:\` ${petProfile.feedCount}\n\`🥤 Drink Count:\` ${petProfile.drinkCount}\n\`🫧 Cleaned Count:\` ${petProfile.cleanedCount}`, inline: false },
      { name: '**Care & Socialisation**', value: `\`👩‍⚕️ Vet Visits:\` ${petProfile.vetCount}\n\`🧑‍🤝‍🧑 Friends:\` ${petProfile.socialisation.friends.length}\n\`🏆 Competitions Entered:\` ${petProfile.socialisation.competitionsEntered}`, inline: false }
    )
    .setColor('#9e38fe');
}
